...
RUN CGO_ENABLED=0 GOOS=linux go build -a -gcflags='-N -l' -installsuffix cgo -o monthly-tier-review ./cmd/monthly-tier-review
# Stage 2: Runtime
FROM alpine:latest
...
COPY --from=build /app/monthly-tier-review /app/
